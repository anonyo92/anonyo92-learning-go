 ...
 * from the main() goroutine to a worker goroutine. When we have no more jobs for the worker 
...
func RunCloseChannel() {
	...
	go func() {
		/* The worker goroutine repeatedly receives from jobs with j, more := <-jobs. 
