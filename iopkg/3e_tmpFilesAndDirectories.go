...
/* Throughout program execution, we often want to create data that
 * isn’t needed after the program exits. Temporary files and directories are useful
...
func RunTmpFilesAndDirectories() {
	...
	f, err := os.CreateTemp("", "golang-test-tmpfile")
	check(err)
	/* The OS is likely to clean up temporary files by itself after some time,
	 ...
	 * and the rest is chosen automatically to ensure that
	...
	dname, err := os.MkdirTemp("", "golang-test-tmpdir")
	check(err)
